...
public class GameView extends JPanel implements ActionListener{
	...
	public void move(double step) {
		// Add step to playerRow to move the plane.
		playerRow += step;
		 // Check if step is positive, negative, or zero and update shipMovement.
		shipMovement = Integer.compare((int) step, 0);
		System.out.println("Move" + shipMovement);
		}
	...
	private void autoMove() {
		...
		try {
			...
			double[] train = sample();
			// Move the plane using the neural network's output.
			move(nn.process(train, Output.NUMERIC_ROUNDED));
			System.out.println("AutoMove Try" + shipMovement);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
			System.out.println("AutoMove Catch" + shipMovement);
		}
	}
	...
	public double[] sample() {
		int numColumns = 3; // Number of columns to consider
	    double[] vector = new double[numColumns * 2 + 1]; // Two values (top, bottom) for each column + playerRow

	    // Iterate over each column
	    for (int i = 0; i < numColumns; i++) {
	        byte[] column = model.get(PLAYER_COLUMN + 1 + i);

	        int top = countTop(column);
	        int bottom = countBottom(column);

	        // Assign values to the vector
	        vector[i * 2] = bottom / 20.0;
	        vector[i * 2 + 1] = top / 20.0;
	    }

	    // Add playerRow to the vector
	    vector[numColumns * 2] = playerRow / 20.0;
	...
	private int countTop(byte[] column) {
	    int top = 0;
	    byte prevValue = 1;

	    // Count continuous '1's from the top of the column
	    for (int i = 0; i < MODEL_HEIGHT; i++) {
	        if (column[i] == 1 && prevValue == 1) {
	            top++;
	        } else {
	            break;
	        }
	...
	private int countBottom(byte[] column) {
	    int bottom = 0;
	    byte prevValue = 1;

	    // Count continuous '1's from the bottom of the column
	...
	public void sampleAndSave(String dataFilename, String expectedFilename) {
	    ...
	    try (FileWriter dataWriter = new FileWriter(dataFilename, true);
	        ...
	        // Save the player movement to the expected file
	        expectedWriter.write(shipMovement + "\n");
	        shipMovement = 0;
	        System.out.println("ShipMovement 0" + shipMovement);

	    } catch (IOException e) {
	        // Handle IO exceptions, e.g., log the error or inform the user
	        e.printStackTrace();
	    }
	}
